...
const useFiltersAvailableForProdId = (
  ...
  const endpoint = `${import.meta.env.VITE_API_BASE_URL as string}/filter/getAllFilterAvailableForProdId`;
  const params = {
    prodId: 0,
    maxFilterId: 0,
    limit: 100,
  ...
  const { isLoading, data: filterData = null, refetch } = useQuery<
    AxiosResponse<GetAllFiltersAvailableForProdIdResponse, unknown>,
    unknown,
    Filter[]
  ...
  return { isLoading, filterData, refetch: refetch as unknown as () => Promise<void> };
};
